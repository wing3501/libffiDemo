...
/// 把函数签名字符串解析到参数数组、返回值字符串中
/// 例子：v24@0:8i16f20---->  返回值v 参数 @ : i f
- (void)_parse
{
    ...
    for (int i = 0; i < _types.length; i ++) {
        ...
        if (c == '^') {
            ...
            [_argumentTypes removeLastObject];//把上一个误添加的@移除，把@?一起添加
...
/// 把用逗号分隔的类型列表组装成函数签名字符串
/// 例子：void,id,SEL,int,float ---->v1@?0@8:8i4f4
...
+ (NSMutableDictionary *)registeredStruct {
    if (_registeredStruct) {
        _registeredStruct = @{@"CGVector":@{
                              @"name": @"CGVector",
                              @"types": @"FF",
                              @"keys": @[@"dx", @"dy"]
                              },
                              ...
                              @"UIEdgeInsets":@{@"name": @"UIEdgeInsets",
                              @"types": @"FFFF",
                              @"keys": @[@"top", @"left", @"bottom", @"right"]
                              },
                              @"UIOffset":@{@"name": @"UIOffset",
                              @"types": @"FF",
